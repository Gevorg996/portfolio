...
$(document).ready(function() {
        ...
        if (regex.test(newname)) {
            if (newname === '') {
                ...
                $.ajax({
                    url: "server.php",
                    type: "post",
                    data: { phone: phone, newname: newname, action: 'ajax6' },
                    success: function(r6) {
                        console.log(r6);
                        if (r6 === "good") {
    ...
    $('.edit-image-btn').click(function() {
        ...
        if (newImage) {
            // Проверяем, что URL начинается с "images/" и содержит разрешение изображения
            if (newImage.startsWith('images/') && (newImage.endsWith('.jpg') || newImage.endsWith('.png') || newImage.endsWith('.jpeg'))) {
                $.ajax({
                    ...
                    success: function(response) {
                        alert(response);
                        // Обновить отображаемое изображение в таблице немедленно (по желанию)
